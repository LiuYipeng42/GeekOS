...
static void Timer_Interrupt_Handler(struct Interrupt_State* state) {
    ...
    if (current->numTicks >= g_Quantum) {
        ...
        // 每经过一个时间片，等待队列中的进程的优先级增加一级
        struct Kernel_Thread *kthread = s_waitingQueue.head;
        while (kthread != 0) {
            if(kthread->pid != IdleThread->pid && kthread->currentReadyQueue > 0)
