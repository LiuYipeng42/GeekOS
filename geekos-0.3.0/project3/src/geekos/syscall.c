...
static int Sys_Spawn(struct Interrupt_State* state) {
    ...
    res = Spawn(program, command,
                &process);  //得到进程名称和用户命令后便可生成一个新进程
...
static int Sys_SetSchedulingPolicy(struct Interrupt_State* state) {
    // project3 Sys_SetSchedulingPolicy
