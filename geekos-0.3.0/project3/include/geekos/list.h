...
#define IMPLEMENT_LIST(LType, NType)                                        \
    ...
    static __inline__ bool Is_Member_Of_##LType(struct LType* listPtr,      \
    ...
    static __inline__ struct NType* Get_Front_Of_##LType(                   \
        struct LType* listPtr) {                                            \
        return listPtr->head;                                               \
    }                                                                       \
    static __inline__ struct NType* Get_Back_Of_##LType(                    \
        struct LType* listPtr) {                                            \
        return listPtr->tail;                                               \
    }                                                                       \
    static __inline__ struct NType* Get_Next_In_##LType(                    \
        struct NType* nodePtr) {                                            \
        return nodePtr->next##LType;                                        \
    }                                                                       \
    static __inline__ void Set_Next_In_##LType(struct NType* nodePtr,       \
                                               struct NType* value) {       \
        nodePtr->next##LType = value;                                       \
    }                                                                       \
    static __inline__ struct NType* Get_Prev_In_##LType(                    \
        struct NType* nodePtr) {                                            \
        return nodePtr->prev##LType;                                        \
    }                                                                       \
    static __inline__ void Set_Prev_In_##LType(struct NType* nodePtr,       \
                                               struct NType* value) {       \
        nodePtr->prev##LType = value;                                       \
    }                                                                       \
    static __inline__ void Add_To_Front_Of_##LType(struct LType* listPtr,   \
                                                   struct NType* nodePtr) { \
    ...
    static __inline__ void Add_To_Back_Of_##LType(struct LType* listPtr,    \
                                                  struct NType* nodePtr) {  \
        KASSERT(!Is_Member_Of_##LType(listPtr, nodePtr));                   \
        nodePtr->next##LType = 0;                                           \
        if (listPtr->tail == 0) {                                           \
    ...
    static __inline__ void Append_##LType(struct LType* listToModify,       \
                                          struct LType* listToAppend) {     \
    ...
    static __inline__ struct NType* Remove_From_Front_Of_##LType(           \
        struct LType* listPtr) {                                            \
        struct NType* nodePtr;                                              \
    ...
    static __inline__ void Remove_From_##LType(struct LType* listPtr,       \
                                               struct NType* nodePtr) {     \
        KASSERT(Is_Member_Of_##LType(listPtr, nodePtr));                    \
