...
static __inline__ bool Check_Thread(struct Thread_Queue *queue, struct Kernel_Thread *kthread) {
    // project3
    return Is_Member_Of_Thread_Queue(queue, kthread);
}
...
extern struct Kernel_Thread* IdleThread;

extern struct Thread_Queue s_waitingQueue;
