 ...
 *   state->edi == stdin file descriptor in low 16 bits, stdout file descriptor
 * in high 16 bits Returns: pid of process if successful, error code (< 0)
 * otherwise
 */
static int Sys_Spawn(struct Interrupt_State* state) {
    ...
    name = Malloc(state->ecx + 1);
    if (!name)
        goto last;
    cmd = Malloc(state->esi + 1);
    if (!cmd)
        goto last;

    if (!Copy_From_User(name, state->ebx, state->ecx))
        goto last;
    if (!Copy_From_User(cmd, state->edx, state->esi))
        goto last;
...
void PrintAllSemaphores() {
    ...
    for (k = 0; k < MAX_SEM; k++) {
        if (g_semaphores[k].sem_id) {
            Print("%5d %6d %8d %s\n", g_semaphores[k].sem_id,
                  g_semaphores[k].registered_users, g_semaphores[k].sem_count,
                  g_semaphores[k].sem_name);
        }
    }
...
static int Sys_CreateSemaphore(struct Interrupt_State* state) {
    ...
    for (k = 0; k < MAX_SEM; k++)
        if (g_semaphores[k].sem_id &&
            !strcmp(g_semaphores[k].sem_name, sem_name)) {
            ...
            struct ThreadsSemaphore* newSemaphore =
                (struct ThreadsSemaphore*)Malloc(
                    sizeof(struct ThreadsSemaphore));
            newSemaphore->semaphore = &g_semaphores[k];
            Add_To_Back_Of_Thread_Semaphore_List(
                &(Get_Current()->SemaphoreList), newSemaphore);
...
static int Sys_Open(struct Interrupt_State* state) {
    ...
    //  Hex_Dump (g_currentThread->userContext->iob, sizeof(struct File*) *
    //  USER_MAX_FILES); Print ("Sys_Open: fd=%d path=%s\n", fileNum, path);
    if (path)
        Free(path);
    if (rc >= 0)
        return fileNum;
...
static int Sys_OpenDirectory(struct Interrupt_State* state) {
    ...
    //  Hex_Dump (g_currentThread->userContext->iob, sizeof(struct File*) *
    //  USER_MAX_FILES); Print("Sys_OpenDirectory: fd=%d path=%s\n", fileNum,
    //  path);
    if (path)
        Free(path);
    if (rc >= 0)
...
static int Sys_FStat(struct Interrupt_State* state) {
    ...
    int rc = 0;
    struct File* pFile =
        (struct File*)g_currentThread->userContext->iob[state->ebx];
 ...
 * It creates the MQ if it doesn't exist, and returns the already-existing id if
 * the MQ exists.
 */
...
static int Sys_MessageQueueReceive(struct Interrupt_State* state) {
    ...
    if (!Copy_To_User(state->ecx, msg->data,
                      state->edx > msg->size ? msg->size : state->edx))
        rc = EUNSPECIFIED;
