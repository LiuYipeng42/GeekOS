...
static int PFAT_Read(struct File* file, void* buf, ulong_t numBytes) {
    ...
    // Print ("PFAT_Read: 1 buf=%ld\n", numBytes);
    if (numBytes > INT_MAX)
        return EINVALID;

    // Print ("PFAT_Read: 2 start=%lu end=%lu fp=%ld ep=%lu numBytes=%lu\n",
    //       start, end, file->filePos, file->endPos, numBytes);
    ...
    //	return EINVALID;
    //  }
    if (start >= file->endPos || end < start)
        return 0;
    ...
    startBlock = Round_Down_To_Block(start) / SECTOR_SIZE;
    ...
    for (i = 0; i < endBlock; ++i) {
        ...
        if (i >= startBlock) {
            ...
            if (!Is_Bit_Set(pfatFile->validBlockSet, i)) {
                ...
                Debug("Reading file block %lu (device block %lu) to %p\n", i,
                      curBlock, pfatFile->fileDataCache + i * SECTOR_SIZE);
                rc = Block_Read(file->mountPoint->dev, curBlock,
                                pfatFile->fileDataCache + i * SECTOR_SIZE);
     ...
     */
    Debug("Copying file from %p to %p, length=%ld\n",
          pfatFile->fileDataCache + start, buf, numBytes);
...
static struct File_Ops s_pfatFileOps = {
    &PFAT_FStat, &PFAT_Read,  &PFAT_Write,
    &PFAT_Seek,  &PFAT_Close, 0, /* Read_Entry */
    &PFAT_Clone,
};
 ...
 */
static directoryEntry* PFAT_Lookup(struct PFAT_Instance* instance,
                                   const char* path) {
 ...
 */
static int PFAT_Open(struct Mount_Point* mountPoint,
                     const char* path,
                     int mode,
                     struct File** pFile) {
    int rc = 0;
...
static int PFAT_Open_Directory(struct Mount_Point* mountPoint,
    ...
    dir->endPos =
        instance->fsinfo.rootDirectoryCount; /* number of directory entries */
    dir->fsData = 0;
 ...
 */
static int PFAT_Stat(struct Mount_Point* mountPoint,
                     const char* path,
...
static int PFAT_Mount(struct Mount_Point* mountPoint) {
    ...
    if (fsinfo->fileAllocationOffset <= 0 ||
        fsinfo->fileAllocationLength <= 0 || fsinfo->rootDirectoryCount < 0 ||
    ...
    rootDirSize =
        Round_Up_To_Block(sizeof(directoryEntry) * fsinfo->rootDirectoryCount);
    instance->rootDir = (directoryEntry*)Malloc(rootDirSize);

    /* Read the root directory */
    Debug("Root directory size = %d\n", rootDirSize);
    int blockNum = fsinfo->rootDirectoryOffset;
    for (i = 0; i < rootDirSize; i += SECTOR_SIZE, blockNum++) {
        if ((rc = Block_Read(mountPoint->dev, blockNum,
                             (void*)instance->rootDir + i)) < 0)
            goto fail;
    }
