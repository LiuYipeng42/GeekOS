...
ulong_t g_PageAllocated = 0;
ulong_t g_PageFreed = 0;
...
void Init_Mem(struct Boot_Info* bootInfo) {
    ...
    KASSERT(HIGHMEM_START == KERN_STACK + PAGE_SIZE);
...
struct Page* Find_Page_To_Page_Out() {
    int i;
    static struct Page* last = NULL;
    struct Page* curr;
    struct Page* best = NULL;
    pte_t* e;

    switch (g_currentPagingAlgorithm) {
        case PAGING_WSCLOCK: /* 类 LRU algorithm */
            if (!last)
                last = g_pageList;
            curr = last + 1;
            i = 0;
            while (
                i++ <
                s_numPages) {  //寻找未使用时间超过WORKING_SET_TIMEOUT阈值的页面
                if (curr >= g_pageList + s_numPages)
                    curr = g_pageList;
                if (curr->flags & PAGE_PAGEABLE) {
                    e = curr->entry;
                    if (g_numTicks - curr->clock > WORKING_SET_TIMEOUT) {
                        best = curr;
                        break;
                    }
                }
            }
            if (!best) { /* 找不到,则随机淘汰一个页面*/
                static ulong_t next = 1;
                next = next * 1103515245L + 23333;
                best = g_pageList + next % s_numPages;
                while (!(best->flags & PAGE_PAGEABLE)) {
                    best++;
                    if (best >= g_pageList + s_numPages)
                        best = g_pageList;
                }
                //            Print ("Random page selected: %d\n", best -
                //            g_pageList);
            }
            last = best;
            break;
...
/**
 * Allocate a page of non pageable physical memory. It works like
 * subroutine Allocate_Pageable_Page(). This subroutine is used to
 * allocate additional pages for a user's page table.
 */
void* Alloc_Resident_Page(void) {
    bool iflag;
    void* paddr = 0;
    struct Page* page = 0;
    ...
    if (paddr != 0) {
        ...
        if (pagefileIndex < 0)
            ...
            goto done;
        Debug("ARP: Free disk page at index %d\n", pagefileIndex);

        /* Make the page temporarily unpageable (can't let another process steal
         * it) */
        page->flags &= ~(PAGE_PAGEABLE);
        ...
        /* Write the page to disk. Interrupts are enabled, since the I/O may
         * block. */
        Debug("ARP: Writing physical frame %p (0x%08lx) to paging file at %d\n",
              paddr, page->vaddr, pagefileIndex);
        Enable_Interrupts();
        Write_To_Paging_File(paddr, page->vaddr, pagefileIndex);
        Disable_Interrupts();

        /* While we were writing got notification this page isn't even needed
         * anymore */
...
void* Alloc_Pageable_Page(pte_t* entry, ulong_t vaddr) {
    ...
    if (paddr != 0) {
        ...
        /* Write the page to disk. Interrupts are enabled, since the I/O may
         * block. */
        Debug("Writing physical frame %p (0x%08lx) to paging file at %d\n",
              paddr, page->vaddr, pagefileIndex);
        Enable_Interrupts();
        Write_To_Paging_File(paddr, page->vaddr, pagefileIndex);
        Disable_Interrupts();
...
void Free_Page(void* pageAddr) {
    ...
    /* When a page is locked, don't free it just let other thread know its not
     * needed */
