    ...
    for (d = 0; d < NUM_PAGE_DIR_ENTRIES; d++) {
        if (dir[d].present) {
            ...
            for (t = 0; t < NUM_PAGE_TABLE_ENTRIES; t++) {
                if (tab[t].present) {
                    Print(
                        "    t=%4d f=%x a=%x d=%x  at=%x gp=%x inf=%x "
                        "addr=%08lx\n",
                        t, tab[t].flags, tab[t].accesed, tab[t].dirty,
                        tab[t].pteAttribute, tab[t].globalPage,
                        tab[t].kernelInfo,
...
static struct User_Context* Create_User_Context(ulong_t size) {
    ...
    for (d = 0; d < NUM_PAGE_DIR_ENTRIES; d++) {
        if (g_dir[d].present) {
            ...
            KASSERT(tab != 0);
            // check tab != 0 without assertion, free allocated pages and return
            // 0.
            dir[d].present = 1;
            ...
            for (t = 0; t < NUM_PAGE_TABLE_ENTRIES; t++) {
                if ((d << 10) + t >= min_addr && (d << 10) + t <= max_addr) {
                    ...
                    tab[t].kernelInfo = 0;
                    tab[t].pageBaseAddr =
                        (ulong_t)Alloc_Pageable_Page(
                            tab + t, ((d << 10) + t) << PAGE_POWER) >>
                        PAGE_POWER;
                    KASSERT(tab[t].pageBaseAddr != 0);
                    // check page address != 0 without assertion, free allocated
                    // pages and return 0.
                    Debug(
                        "Allocate page, uc=0x%08lx, didx=%4d, tbix=%4d, "
                        "pg=0x%08x va=0x%08x\n",
    ...
    /* allocate space for one page of stack memory at the end of the virtual
     * address range */
    Debug("Allocate stack.\n");
    dir[NUM_PAGE_DIR_ENTRIES - 1].present = 1;
    dir[NUM_PAGE_DIR_ENTRIES - 1].flags =
        VM_WRITE | VM_READ | VM_EXEC | VM_USER;
    tab = (pte_t*)Alloc_Page(); /* page table */
    KASSERT(tab != 0);
    // check tab != 0 without assertion, free allocated pages and return 0.
    dir[NUM_PAGE_DIR_ENTRIES - 1].pageTableBaseAddr =
        (ulong_t)tab >> PAGE_POWER;
    ...
    // check stack page != 0 without assertion, free allocated pages and return
    // 0.
    ...
    Debug(
        "Allocate page, uc=0x%08lx, didx=%4d, tbix=%4d, pg=0x%08lx va=0x%08x\n",
        (ulong_t)uc, NUM_PAGE_DIR_ENTRIES - 1, t, (ulong_t)stack_page,
        USER_STACK_PAGE_ADDR);
    ...
    uc->ldtSelector = Selector(KERNEL_PRIVILEGE, true,
                               Get_Descriptor_Index(uc->ldtDescriptor));
    Init_Code_Segment_Descriptor(&uc->ldt[0], (ulong_t)uc->memory,
                                 uc->size / PAGE_SIZE, USER_PRIVILEGE);
    Init_Data_Segment_Descriptor(&uc->ldt[1], (ulong_t)uc->memory,
                                 uc->size / PAGE_SIZE, USER_PRIVILEGE);
 ...
 */
bool __copy_to_user(struct User_Context* context,
                    ulong_t destInUser,
                    void* srcInKernel,
                    ulong_t numBytes) {
    /*
    ...
    while (a <= e) {
        ...
        if (!dir[PAGE_DIRECTORY_INDEX(anf_p)].present) {
            Print(
                "__copy_to_user: page directory not present: p=0x%08lx "
                "dir=0x%08lx idx=%ld\n",
        ...
        tab = (pte_t*)(dir[PAGE_DIRECTORY_INDEX(anf_p)].pageTableBaseAddr
                       << PAGE_POWER);

        if (!tab[PAGE_TABLE_INDEX(anf_p)].present &&
            tab[PAGE_TABLE_INDEX(anf_p)].kernelInfo != KINFO_PAGE_ON_DISK) {
            Print(
                "__copy_to_user: page table not present: "
        ...
        if (tab[PAGE_TABLE_INDEX(anf_p)].present) {
            ...
            phy_a = (ulong_t)Alloc_Pageable_Page(tab + PAGE_TABLE_INDEX(anf_p),
                                                 (ulong_t)anf_p);
 ...
 * Returns true if successful, false otherwise.
 */
bool __copy_from_user(struct User_Context* context,
                      void* destInKernel,
                      ulong_t srcInUser,
                      ulong_t numBytes) {
    /*
    ...
    while (a <= e) {
        ...
        if (!dir[PAGE_DIRECTORY_INDEX(anf_p)].present) {
            ...
            Print(
                "__copy_from_user: page directory entry not present: "
        ...
        if (!tab[PAGE_TABLE_INDEX(anf_p)].present &&
            tab[PAGE_TABLE_INDEX(anf_p)].kernelInfo != KINFO_PAGE_ON_DISK) {
            Print(
                "__copy_from_user: page table entry not present: "
                "p=0x%08lx dir=0x%08lx didx=%ld tab=0x%08lx tidx=%ld\n",
                anf_p, (ulong_t)dir, PAGE_DIRECTORY_INDEX(anf_p), (ulong_t)tab,
                PAGE_TABLE_INDEX(anf_p));
            End_Int_Atomic(iflag);
...
void Destroy_User_Context(struct User_Context* context) {
    ...
    for (d = 0; d < NUM_PAGE_DIR_ENTRIES; d++) {
        ...
        if (dir[d].present && dir[d].flags & VM_USER) {
            ...
            for (t = 0; t < NUM_PAGE_TABLE_ENTRIES; t++) {
                ...
                } else if (tab[t].kernelInfo == KINFO_PAGE_ON_DISK)
    ...
    // TODO: free semaphores, files, and other resources: -> not here! but in
    // Exit()
...
int Load_User_Program(char* exeFileData,
    ...
    for (i = 0; i < exeFormat->numSegments; i++, s++) {
        ...
        if (segMaxAddr > maxAddr)
            maxAddr = segMaxAddr;
    }

    /* we place the argument block on top of the memory (will be the stack
     * later) */
    ...
    for (i = 0; i < exeFormat->numSegments; i++, s++)
        if (s->lengthInFile > 0)
            __copy_to_user(uc, s->startAddress, exeFileData + s->offsetInFile,
                           s->lengthInFile);
    ...
    uc->stackPointerAddr =
        argv; /* arguments are passed via the stack to main() */
    uc->size = size + INITIAL_HEAP_SIZE; /* maximum code address rounded up to
                                            next page address */
    *pUserContext = uc;
...
bool Copy_From_User(void* destInKernel, ulong_t srcInUser, ulong_t numBytes) {
    ...
    Debug("Copy_From_User: 0x%08lx <- 0x%08lx x 0x%08lx\n",
          (ulong_t)destInKernel, srcInUser, numBytes);
