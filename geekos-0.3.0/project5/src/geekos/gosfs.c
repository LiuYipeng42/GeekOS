...
int Find_Free_Inode(struct Mount_Point* mountPoint, ulong_t* retInode) {
    ...
    for (i = 0; i < GOSFS_NUM_INODES; i++) {
        if (((struct GOSFS_Instance*)(mountPoint->fsData))
                ->superblock.inodes[i]
                .flags == 0) {
            rc = 0;
            *retInode = i;
...
/* 检查目录是否为空 */
bool IsDirectoryEmpty(struct GOSFS_Inode* pInode,
...
int WriteSuperblock(struct GOSFS_Instance* p_instance) {
    ...
    for (i = 0; i < numBlocks; i++) {
        ...
        if ((p_instance->superblock.supersize - bwritten) <
            GOSFS_FS_BLOCK_SIZE) {
            memcpy(p_buff->data, ((void*)&(p_instance->superblock)) + bwritten,
                   p_instance->superblock.supersize - bwritten);
...
int CreateFirstDirectoryBlock(ulong_t thisInode,
                              ulong_t parentInode,
                              struct FS_Buffer* p_buff) {
    ...
    for (i = 0; i < GOSFS_DIR_ENTRIES_PER_BLOCK; i++) {
        ...
        memcpy(p_buff->data + (i * sizeof(struct GOSFS_Directory)), &dirEntry,
               sizeof(struct GOSFS_Directory));
    }
...
/*通过inode删除目录项 */
int RemoveDirEntryFromInode(struct GOSFS_Instance* p_instance,
                            ulong_t parentInode,
                            ulong_t inode) {
    int rc = 0, i = 0, e = 0;
    struct FS_Buffer* p_buff = 0;
...
/* 向inode添加目录项 */
int AddDirEntry2Inode(struct GOSFS_Instance* p_instance,
                      ulong_t parentInode,
                      struct GOSFS_Directory* dirEntry) {
            ...
            for (e = 0; e < GOSFS_DIR_ENTRIES_PER_BLOCK; e++) {
                ...
                if (tmpDir->type == GOSFS_DIRTYP_FREE) {
                    ...
                    memcpy(
                        ...
                        dirEntry, sizeof(struct GOSFS_Directory));
                    found = 1;
                    Modify_FS_Buffer(p_instance->buffercache, p_buff);
                    p_instance->superblock.inodes[parentInode].size++;
                    break;
    ...
    if ((found == 0) && (numDirectPtr >= 0)) {
        blockNum = Find_First_Free_Bit(p_instance->superblock.bitSet,
                                       p_instance->superblock.size);
        ...
        rc = Release_FS_Buffer(p_instance->buffercache, p_buff);
        p_buff = 0;
        p_instance->superblock.inodes[parentInode].blockList[numDirectPtr] =
            blockNum;
        p_instance->superblock.inodes[parentInode].size++;
...
/* 通过文件名或路径名在目录中查找索引节点 */
int Find_InodeInDirectory(struct GOSFS_Instance* p_instance,
                          char* path,
                          ulong_t searchInode,
                          ulong_t* retInode) {
...
//通过给定的文件路径寻找inode
int Find_InodeByName(struct GOSFS_Instance* p_instance,
                     const char* path,
                     ulong_t* retInode) {
    char* searchPath;
    char* nextSlash;
    ...
    while (finished == 0) {
        ...
        searchPath =
            strncpy(searchPath, path + offset, nextSlash - (path + offset));
...
/* 创建一个文件inode */
int CreateFileInode(struct Mount_Point* mountPoint,
                    const char* path,
                    ulong_t* inode) {
    int rc = 0;
    struct GOSFS_Inode* pInode;
    struct GOSFS_Instance* p_instance =
        (struct GOSFS_Instance*)mountPoint->fsData;
    struct GOSFS_Directory dirEntry;
    ...
    pInode->acl[0].uid =
        g_currentThread->userContext ? g_currentThread->userContext->eUId : 0;
...
bool FileBlockExists(struct GOSFS_Instance* p_instance,
    ...
    if (blockNum >= GOSFS_NUM_DIRECT_BLOCKS +
                        ...
                        (GOSFS_NUM_2X_INDIRECT_BLOCKS *
                         GOSFS_NUM_PTRS_PER_BLOCK * GOSFS_NUM_PTRS_PER_BLOCK)) {
        rc = 0;
        goto finish;
    }
    ...
    // 检查间接块
    else if (blockNum < GOSFS_NUM_DIRECT_BLOCKS + (GOSFS_NUM_INDIRECT_BLOCKS *
                                                   GOSFS_NUM_PTRS_PER_BLOCK)) {
        numIndirectPtr = ((((blockNum + 1) - (GOSFS_NUM_DIRECT_BLOCKS + 1)) /
                           GOSFS_NUM_INDIRECT_PTR_PER_BLOCK)) +
                         1;
        numPtrInIndirect = ((((blockNum + 1) - (GOSFS_NUM_DIRECT_BLOCKS + 1)) %
                             GOSFS_NUM_INDIRECT_PTR_PER_BLOCK));
        ...
        memcpy(&phyBlock, p_buff->data + (numPtrInIndirect * sizeof(ulong_t)),
               sizeof(ulong_t));
        rc = Release_FS_Buffer(p_instance->buffercache, p_buff);
        p_buff = 0;
        if (phyBlock > 0)
    ...
    // 检查二次间接块
        ...
        numIndirectPtr =
            ((((blockNum + 1) -
               (GOSFS_NUM_DIRECT_BLOCKS +
                (GOSFS_NUM_INDIRECT_BLOCKS * GOSFS_NUM_PTRS_PER_BLOCK) + 1)) /
              (GOSFS_NUM_INDIRECT_PTR_PER_BLOCK *
               GOSFS_NUM_INDIRECT_PTR_PER_BLOCK))) +
            1;
        numPtrIn2Indirect =
        ...
        numPtrInIndirect =
            ((((blockNum + 1) -
               (GOSFS_NUM_DIRECT_BLOCKS +
                (GOSFS_NUM_INDIRECT_BLOCKS * GOSFS_NUM_PTRS_PER_BLOCK) + 1)) %
              (GOSFS_NUM_INDIRECT_PTR_PER_BLOCK)));
        inodePtr = GOSFS_NUM_DIRECT_BLOCKS +
                   (GOSFS_NUM_INDIRECT_BLOCKS * GOSFS_NUM_PTRS_PER_BLOCK) +
                   numIndirectPtr - 1;
        ...
        rc = Get_FS_Buffer(p_instance->buffercache, indirectBlock, &p_buff);
        memcpy(&phyIndBlock,
               p_buff->data + (numPtrIn2Indirect * sizeof(ulong_t)),
               sizeof(ulong_t));
...
ulong_t GetNewCleanBlock(struct GOSFS_Instance* p_instance) {
    ...
    freeBlock = Find_First_Free_Bit(p_instance->superblock.bitSet,
                                    p_instance->superblock.size);
...
/* 写入间接块  */
int WriteIndirectBlockEntry(struct GOSFS_Instance* p_instance,
                            ulong_t numBlock,
                            ulong_t offset,
                            ulong_t freeBlock) {
    int rc = 0;
    ...
    // lets "format" block
    memcpy(p_buff->data + (offset * sizeof(ulong_t)), &freeBlock,
           sizeof(ulong_t));
...
/* 通过块的线性地址得到块的物理地址 */
int GetPhysicalBlockByLogical(struct GOSFS_Instance* p_instance,
                              struct GOSFS_Inode* inode,
                              ulong_t blockNum) {
    int rc = 0;
    int phyBlock = 0, phyIndBlock = 0;
    int numIndirectPtr = -1;  // which indirect inode ptr to use (1-based)
    int numPtrInIndirect =
        -1;  // which pointer in the indirct-block (0-based) 一次间接块
    int numPtrIn2Indirect =
        -1;  // which pointer in the 2xindirct-block (0-based) 二次间接块
    int inodePtr =
        -1;  // which entry in the inode-array are we refering to (0-based)
    ...
    // 间接块
             ...
             GOSFS_NUM_DIRECT_BLOCKS + (GOSFS_NUM_INDIRECT_PTR_PER_BLOCK *
                                        GOSFS_NUM_INDIRECT_BLOCKS)) {
        numIndirectPtr = ((((blockNum + 1) - (GOSFS_NUM_DIRECT_BLOCKS + 1)) /
                           GOSFS_NUM_INDIRECT_PTR_PER_BLOCK)) +
                         1;
        numPtrInIndirect = (((blockNum + 1) - (GOSFS_NUM_DIRECT_BLOCKS + 1)) %
        ...
        inodePtr = GOSFS_NUM_DIRECT_BLOCKS + numIndirectPtr - 1;
        Debug(
            "GetPhysicalBlockByLogical: blocknum: %ld, numIndirectPtr: %d, "
            "numPtrInIndirect: %d, inodePtr: %d\n",
    ...
    // 二次间接块
        ...
        Debug(
            "GetPhysicalBlockByLogical: blocknum: %ld, numIndirectPtr: %d, "
            "numPtrIn2Indirect: %d, numPtrInIndirect: %d, inodePtr: %d\n",
            blockNum, numIndirectPtr, numPtrIn2Indirect, numPtrInIndirect,
            inodePtr);
...
/* 为inode指向的文件分配一个新块 */
int CreateFileBlock(struct GOSFS_Instance* p_instance,
                    struct GOSFS_Inode* inode,
                    ulong_t blockNum) {
    int rc = 0;
    int freeBlock;
    ...
    int numPtrIn2Indirect =
        -1;  // which pointer in the 2xindirct-block (0-based)
    int inodePtr =
        -1;  // which entry in the inode-array are we refering to (0-based)
    int indirectBlock;  // physical block with block-ptrs
    ...
    // 间接块
             ...
             (GOSFS_NUM_INDIRECT_PTR_PER_BLOCK * GOSFS_NUM_INDIRECT_BLOCKS) +
                 GOSFS_NUM_DIRECT_BLOCKS) {
        Debug("using indirect pointer\n");
        numIndirectPtr = (((blockNum - (GOSFS_NUM_DIRECT_BLOCKS + 1)) /
                           GOSFS_NUM_INDIRECT_PTR_PER_BLOCK)) +
                         1;
        numPtrInIndirect = (((blockNum - (GOSFS_NUM_DIRECT_BLOCKS + 1)) %
                             GOSFS_NUM_INDIRECT_PTR_PER_BLOCK));
        inodePtr = GOSFS_NUM_DIRECT_BLOCKS + numIndirectPtr - 1;
        ...
        // 写入间接块
        rc = WriteIndirectBlockEntry(p_instance, indirectBlock,
                                     numPtrInIndirect, freeBlock);

    }
    // 二次间接块
    else if (blockNum <= (GOSFS_NUM_INDIRECT_BLOCKS * GOSFS_NUM_PTRS_PER_BLOCK *
                          GOSFS_NUM_PTRS_PER_BLOCK) +
                             (GOSFS_NUM_INDIRECT_PTR_PER_BLOCK *
                              GOSFS_NUM_INDIRECT_BLOCKS) +
                             GOSFS_NUM_DIRECT_BLOCKS) {
        ...
        // 写入二次间接块
        rc = WriteIndirectBlockEntry(p_instance, phyIndBlock, numPtrInIndirect,
                                     freeBlock);
    } else {
...
static int GOSFS_Read(struct File* file, void* buf, ulong_t numBytes) {
    ...
    Debug(
        "GOSFS_Read: about to read from offs=%ld (startblk=%ld) to end=%ld "
        "(endblk=%ld)\n",
    ...
    for (i = startBlock; i <= endBlock; i++) {
        ...
        phyBlock = GetPhysicalBlockByLogical(pFileEntry->instance,
                                             pFileEntry->inode, i);
        ...
        // read data
        rc =
            Get_FS_Buffer(pFileEntry->instance->buffercache, phyBlock, &p_buff);
        ...
        if (bytesRead + readNum > numBytes)
            readNum = numBytes - bytesRead;
...
finish:
    ...
    if (p_buff != 0)
        Release_FS_Buffer(pFileEntry->instance->buffercache, p_buff);
...
static int GOSFS_Write(struct File* file, void* buf, ulong_t numBytes) {
    ...
    for (i = startBlock; i <= endBlock; i++) {
        ...
        Debug(
            "About to write (logical) blocknumber %ld to physical block %ld\n",
            i, phyBlock);
        ...
        if (i == startBlock)
            writeFrom = startBlockOffset;
        else
            writeFrom = 0;

        writeNum = GOSFS_FS_BLOCK_SIZE - writeFrom;

        if (writeNum > numBytes - bytesWritten)
            writeNum = numBytes - bytesWritten;
...
static struct File_Ops s_gosfsFileOps = {
    &GOSFS_FStat, &GOSFS_Read,  &GOSFS_Write,
    &GOSFS_Seek,  &GOSFS_Close, 0, /* Read_Entry */
    &GOSFS_Clone,
};
...
static int GOSFS_FStat_Directory(struct File* dir, struct VFS_File_Stat* stat) {
    int rc = 0;
    struct GOSFS_FileEntry* fileEntry =
        0;  // (struct GOSFS_FileEntry*) dir->fsData;
...
static int GOSFS_Read_Entry(struct File* dir, struct VFS_Dir_Entry* entry) {
    ...
    inode = &(((struct GOSFS_Instance*)(dir->mountPoint->fsData))
                  ->superblock.inodes[directory->inode]);
...
static int GOSFS_Open(struct Mount_Point* mountPoint,
    ...
    struct File* file = Allocate_File(&s_gosfsFileOps, 0, pInode->size,
                                      pFileEntry, mode, mountPoint);
    if (file == 0) {
        rc = ENOMEM;
        goto finish;
    }
    ...
    *pFile = file;
    ...
    Debug("GOSFS_Open: pid=%d ref=%d, fda=%p\n", g_currentThread->pid,
          pFileEntry->references, pFileEntry);

finish:
    if ((rc < 0) && (pFileEntry != 0))
        Free(pFileEntry);
    Mutex_Unlock(&p_instance->lock);
    return rc;
}
 ...
 * 创建一个指定路径的目录
 */
static int GOSFS_Create_Directory(struct Mount_Point* mountPoint,
                                  const char* path) {
    int rc = 0, e;
    ulong_t freeBlock;
    struct GOSFS_Directory dirEntry;
    ...
    p_instance->superblock.inodes[freeInode].size =
        2;  // directories start with 2 entries ("." and "..")
    p_instance->superblock.inodes[freeInode].link_count = 1;
    p_instance->superblock.inodes[freeInode].blocks_used =
        1;  // new directories will consume 1 Block
    p_instance->superblock.inodes[freeInode].flags =
        GOSFS_INODE_ISDIRECTORY | GOSFS_INODE_USED;
    memset(p_instance->superblock.inodes[freeInode].acl, '\0',
           sizeof(struct VFS_ACL_Entry) * VFS_MAX_ACL_ENTRIES);
 ...
 * 通过指定路径打开文件夹
 */
static int GOSFS_Open_Directory(struct Mount_Point* mountPoint,
                                const char* path,
                                struct File** pDir) {
    int rc = 0, i, e;
    ...
    if (strcmp(path, "/") == 0) {
        ...
        rc = Find_InodeByName((struct GOSFS_Instance*)mountPoint->fsData, path,
                              &inodeNum);
        if (rc < 0)
            goto finish;
    }

    inode = &(((struct GOSFS_Instance*)mountPoint->fsData)
                  ->superblock.inodes[inodeNum]);
    ...
    for (i = 0; i < GOSFS_NUM_DIRECT_BLOCKS; i++) {
        ...
        if (blockNum != 0) {
            Debug("found direct block %ld\n", blockNum);
            rc = Get_FS_Buffer(
                ((struct GOSFS_Instance*)mountPoint->fsData)->buffercache,
                blockNum, &p_buff);
            ...
            for (e = 0; e < GOSFS_DIR_ENTRIES_PER_BLOCK; e++) {
                ...
                if (tmpDir->type != GOSFS_DIRTYP_FREE) {
                    ...
                    memcpy(dirEntries + found, tmpDir,
                           sizeof(struct GOSFS_Directory));
                    found++;
                }
            }

            rc = Release_FS_Buffer(
                ((struct GOSFS_Instance*)mountPoint->fsData)->buffercache,
                p_buff);
            p_buff = 0;
        }
...
finish:
    if ((rc < 0) && (dirEntries != 0))
        Free(dirEntries);
    if (p_buff != 0)
        Release_FS_Buffer(
            ((struct GOSFS_Instance*)mountPoint->fsData)->buffercache, p_buff);
    Mutex_Unlock(&p_instance->lock);
    return rc;
}
...
static int GOSFS_Delete(struct Mount_Point* mountPoint, const char* path) {
    ...
    for (i = 0; i < GOSFS_NUM_INDIRECT_BLOCKS; i++) {
        ...
        if (blockNum != 0) {
            ...
            for (e = 0; e < GOSFS_NUM_INDIRECT_PTR_PER_BLOCK; e++) {
                memcpy(&blockIndirect,
                       (void*)p_buff->data + (e * sizeof(ulong_t)),
                       sizeof(ulong_t));
                if (blockIndirect != 0) {
    ...
    for (i = 0; i < GOSFS_NUM_2X_INDIRECT_BLOCKS; i++) {
        blockNum = pInode->blockList[GOSFS_NUM_DIRECT_BLOCKS +
                                     GOSFS_NUM_INDIRECT_BLOCKS + i];
        if (blockNum != 0) {
            Debug("found indirect block %ld --> freeing\n", blockNum);

            for (e = 0; e < GOSFS_NUM_INDIRECT_PTR_PER_BLOCK; e++) {
                rc = Get_FS_Buffer(p_instance->buffercache, blockNum, &p_buff);
                memcpy(&block2Indirect,
                       (void*)p_buff->data + (e * sizeof(ulong_t)),
                       sizeof(ulong_t));
                rc = Release_FS_Buffer(p_instance->buffercache, p_buff);
                p_buff = 0;
 ...
 */
static int GOSFS_Stat(struct Mount_Point* mountPoint,
                      const char* path,
    ...
    stat->size = ((struct GOSFS_Instance*)mountPoint->fsData)
                     ->superblock.inodes[inode]
                     .size;
...
static struct Mount_Point_Ops s_gosfsMountPointOps = {
    &GOSFS_Open, &GOSFS_Create_Directory, &GOSFS_Open_Directory, &GOSFS_Stat,
...
static int GOSFS_Format(struct Block_Device* blockDev) {
    ...
    ulong_t byteCountSuperblock =
        sizeof(struct GOSFS_Superblock) + FIND_NUM_BYTES(numBlocks);
    // 需要块的数量
    ...
    for (i = 0; i < blockCountSuperblock; i++) {
        ...
        if ((byteCountSuperblock - bcopied) < GOSFS_FS_BLOCK_SIZE) {
            memcpy(p_buff->data, ((void*)superblock) + bcopied,
                   byteCountSuperblock - bcopied);
...
static int GOSFS_Mount(struct Mount_Point* mountPoint) {
    ...
    if (superblock->magic != GOSFS_MAGIC) {
        Print(
            "GOSFS_Mount ERROR: does not seem to be a GOSFS filesystem, try "
            "format first\n");
        rc = EFSGEN;
        goto finish;
    }
    ...
    /* 创建文件系统实例 */
    int sizeofInstance =
        sizeof(struct GOSFS_Instance) + FIND_NUM_BYTES(superblock->size);
    ...
    for (i = 0; i < numBlocks; i++) {
        ...
        if ((numBytes - bwritten) < GOSFS_FS_BLOCK_SIZE) {
            memcpy(((void*)superblock) + bwritten, p_buff->data,
                   numBytes - bwritten);
