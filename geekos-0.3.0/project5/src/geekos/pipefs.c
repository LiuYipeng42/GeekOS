...
struct Pipe {
    ...
    uint_t references; /* the number of filedescriptors that references this
                          pipe  */
...
int Create_Pipe(struct File** pRead, struct File** pWrite) {
    ...
    if ((read = Allocate_File(&s_readPipeFileOps, 0, 0, p, O_READ, 0)) == 0 ||
        (write = Allocate_File(&s_writePipeFileOps, 0, 0, p, O_WRITE, 0)) ==
            0) {
        rc = ENOMEM;
    }
