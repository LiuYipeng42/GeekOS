...
#ifdef DEBUG
#ifndef KTHREAD_DEBUG
#define KTHREAD_DEBUG
#endif
#endif
 ...
 */
static void Init_Thread(struct Kernel_Thread* kthread,
                        void* stackPage,
                        int priority,
                        bool detached) {
...
/*static*/ void Setup_User_Thread(struct Kernel_Thread* kthread,
    ...
    Debug(
        "Setup_User_Thread() pid=%d, thread=0x%08lx, stack=0x%08lx, "
        "uc=0x%08lx\n",
        kthread->pid, (ulong_t)kthread, (ulong_t)kthread->stackPage,
        (ulong_t)userContext);

    Attach_User_Context(kthread, userContext);

    Push(kthread, userContext->dsSelector);       /* stack data selector */
    Push(kthread, userContext->stackPointerAddr); /* stack pointer */

    Push(kthread, EFLAGS_IF);               /* eflags */
    Push(kthread, userContext->csSelector); /* text selector */
    Push(kthread, userContext->entryAddr);  /* program counter */

    Push(kthread, 0); /* error code */
    Push(kthread, 0); /* interrupt number */

    Push(kthread, userContext->size - INITIAL_HEAP_SIZE); /* eax */
    Push(kthread, 0);                                     /* ebx */
    Push(kthread, INITIAL_HEAP_SIZE);                     /* ecx */
    Push(kthread, 0);                                     /* edx */
    ...
    while (true) {
        ...
        if ((kthread = s_graveyardQueue.head) == 0) {
            ...
            while (kthread != 0) {
                ...
                Debug(
                    "Reaper: pid=%d disposing of thread @ 0x%08x, stack @ "
                    "0x%08x\n",
                    kthread->pid, (uint_t)kthread, (uint_t)kthread->stackPage);
...
/*
 * Print list of processes to screen
 */
#define MAX_CMD_LEN 64
int PrintProcessList(void) {
    struct User_Context* uc;
    struct Kernel_Thread* kthread;
    char* cmd;
    char buf[MAX_CMD_LEN];
    char* n[6] = {"",         "(mainThread)",      "(Idle)",
                  "(Reaper)", "(Floppy Requests)", "(IDE Requests)"};
    int uid;

    kthread = Get_Front_Of_All_Thread_List(&s_allThreadList);

    Print("PID UID Ticks Priority RefCount ReadyQueue Blocked Command\n");

    while (kthread) {
        uid = 0;
        uc = kthread->userContext;
        if (uc) {
            ulong_t pargv = uc->argBlockAddr + sizeof(int);
            ulong_t argv = 0;
            __copy_from_user(uc, &argv, pargv, sizeof(char**));
            __copy_from_user(uc, &argv, argv, sizeof(char*));
            Debug(
                "PPL: mem=0x%08lx arg=0x%08lx pargv=0x%08lx argv=0x%08lx "
                "sz=%d\n",
                (ulong_t)uc->memory, uc->argBlockAddr, pargv, argv,
                sizeof(int) + sizeof(char**));
            int i = 0;
            *buf = '\0';
            for (i = 0;
                 __copy_from_user(uc, buf + i, argv + i, 1) && i < MAX_CMD_LEN;
                 i++)
                if (buf[i] == '\0')
                    break;
            buf[MAX_CMD_LEN - 1] = '\0';
            ...
            cmd = (kthread->pid >= 1 && kthread->pid <= 5) ? n[kthread->pid]
                                                           : "(kernel)";

        Print("%3d %3d %5ld %8d %8d %10d %7d %s\n", kthread->pid, uid,
              kthread->numTicks, kthread->priority, kthread->refCount,
              kthread->currentReadyQueue, kthread->blocked, cmd);
...
void Init_Scheduler(void) {
    ...
    Debug("Initialized mainThread, pid=%d\n", mainThread->pid);
    ...
    Debug("Started idle thread, pid=%d\n", idle->pid);
    ...
    Debug("Started reaper thread, pid=%d\n", reaper->pid);
}
...
struct Kernel_Thread* Start_User_Thread(struct User_Context* userContext,
    ...
    Debug("Start_User_Thread()\n");
    struct Kernel_Thread* t = Create_Thread(PRIORITY_USER, detached);

    if (t) {
        Setup_User_Thread(t, userContext);
        Make_Runnable_Atomic(t);
    }
...
void Destroy_Threads_Semaphores(struct Kernel_Thread* kthread) {
    bool iflag = Begin_Int_Atomic();

#ifdef SEMAPHORE_DEBUG
    Print("About to destroy all Semaphores for process %d\n", kthread->pid);
#endif
    struct ThreadsSemaphore* kthread_sem =
        Get_Front_Of_Thread_Semaphore_List(&(kthread->SemaphoreList));
    while (kthread_sem != 0) {
#ifdef SEMAPHORE_DEBUG
        // Print("Found semaphore %s (%d)\n",kthread_sem->semaphore->sem_name,
        // kthread_sem->semaphore->sem_id);
        Print("Found semaphore (%d)\n", kthread_sem->semaphore->sem_id);
#endif
        DestroySemaphore(kthread_sem->semaphore->sem_id, kthread);
        // Attention: Get_Next cannot be used here, because DestroySemaphore
        // frees kthread_sem
        kthread_sem =
            Get_Front_Of_Thread_Semaphore_List(&(kthread->SemaphoreList));
    }
...
void Exit(int exitCode) {
    ...
    /* Destroy used semaphores if this thread */
    Destroy_Threads_Semaphores(current);
...
struct Kernel_Thread* Lookup_Thread(int pid) {
    ...
    t = Get_Front_Of_All_Thread_List(&s_allThreadList);
    while (t != 0) {
        if (t->pid == pid) {
            if (g_currentThread == t->owner)
                result = t;
            break;
        }
        t = Get_Next_In_All_Thread_List(t);
    }

    End_Int_Atomic(iflag);

    Debug("Lookup_Thread(): pid=%d, result=%08X\n", pid, (unsigned int)result);
...
void Wake_Up_One(struct Thread_Queue* waitQueue) {
    ...
    if (best != 0) {
        ...
        Make_Runnable(best);
        Debug("Wake_Up_One: waking up %08X from %08X\n", (unsigned int)best,
              (unsigned int)g_currentThread);
    }
}
...
void Dump_All_Thread_List(void) {
    ...
    while (kthread != 0) {
        ...
        Print("<%lx,%lx,%lx>", (ulong_t)Get_Prev_In_All_Thread_List(kthread),
              (ulong_t)kthread, (ulong_t)Get_Next_In_All_Thread_List(kthread));
...
// Get the first thread of all the existing threads
struct Kernel_Thread* Get_FirstOfAllThreads(void) {
    return Get_Front_Of_All_Thread_List(&s_allThreadList);
}

// Get next thread of all the existing threads
struct Kernel_Thread* Get_NextOfAllThreads(struct Kernel_Thread* kthread) {
    return Get_Next_In_All_Thread_List(kthread);
}
