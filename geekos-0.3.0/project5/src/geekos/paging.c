...
int g_currentPagingAlgorithm =
    PAGING_DEFAULT; /* currently used paging algorithm */
ulong_t g_WorkingSet_Timeout =
    WORKING_SET_TIMEOUT; /* maximum page age in working set */
...
void Hex_Dump(void* paddr, ulong_t len) {
    ...
    for (k = 0; k < len; k += 16) {
        bool all_00 = true;
        for (x = 0; x < 16; x++)
            if (*(uchar_t*)(paddr + k + x) != 0x00)
                all_00 = false;
        bool all_55 = true;
        for (x = 0; x < 16; x++)
            if (*(uchar_t*)(paddr + k + x) != 0x55)
                all_55 = false;
...
void Dump_Paging_Info(void) {
    ...
    Print(
        "RAM Pages: total=%d, free=%d, faults=%ld, allocated=%ld, freed=%ld\n"
...
static void Page_Fault_Handler(struct Interrupt_State* state) {
    ...
    if ((address >= 0 && address < PAGE_SIZE) ||
        (address >= USER_BASE_ADDRESS &&
         address < USER_BASE_ADDRESS + PAGE_SIZE)) {
    ...
    if (!faultCode.protectionViolation && faultCode.userModeFault) {
        ++g_PageFaults;
        pde_t* dir = g_currentThread->userContext->pageDir +
                     PAGE_DIRECTORY_INDEX(address);
        pte_t* tab = 0;

        if (dir->present) {
            tab = (pte_t*)(dir->pageTableBaseAddr << PAGE_POWER) +
                  PAGE_TABLE_INDEX(address);
        } else {
            int t;
            Debug(
                "PF: allocating new page table at directory index %ld "
                "##################\n",
                PAGE_DIRECTORY_INDEX(address));
        ...
        if (tab->kernelInfo == KINFO_PAGE_ON_DISK) {
            ...
            if (paddr != 0) {
                ...
                /* Make the page temporarily unpageable (can't let another
                 * process steal it) */
                page->flags &= ~(PAGE_PAGEABLE);
                ...
                /* Write the page to disk. Interrupts are enabled, since the I/O
                 * may block. */
                Debug(
                    "PF: Writing physical frame %p (0x%08lx) to paging file at "
                    "%d, flags=%x\n",
                ...
                /* While we were writing got notification this page isn't even
                 * needed anymore */
                if (page->flags & PAGE_ALLOCATED) {
                    ...
                    page->entry->pageBaseAddr =
                        pagefileIndex; /* Remember where it is located! */
                } else {
            ...
            /* Read the page from disk. Interrupts are enabled, since the I/O
             * may block. */
            Debug(
                "PF: Reading physical frame %p (0x%08lx) from paging file at "
                "%d, flags=%x\n",
            ...
            /* Free the page in paging file, since we read the content into
             * memory */
            ...
            tab->kernelInfo = 0;
            tab->pageBaseAddr =
                (ulong_t)Alloc_Pageable_Page(tab, PAGE_ADDR(address)) >>
                PAGE_POWER;
            KASSERT(tab->pageBaseAddr != 0);
            Debug(
                "\nAllocate additional page, uc=0x%08lx, pg=0x%08x "
                "va=0x%08lx\n",
                (ulong_t)g_currentThread->userContext,
                tab->pageBaseAddr << PAGE_POWER, address);
            Flush_TLB();
        }
        return;
    }
...
void Init_Paging(void) {
    ...
    s_PageFileAdm =
        (unsigned char*)Malloc(pageDev->numSectors / SECTORS_PER_PAGE);
    int i, mx = pageDev->numSectors / SECTORS_PER_PAGE;
    for (i = 0; i < mx; i++)
        s_PageFileAdm[i] = 0;
}
...
void Free_Space_On_Paging_File(int pagefileIndex) {
    ...
    struct Paging_Device* pageDev = Get_Paging_Device();
    KASSERT(pageDev != 0);
    KASSERT(pagefileIndex >= 0 &&
            pagefileIndex <= pageDev->numSectors / SECTORS_PER_PAGE);
...
void Write_To_Paging_File(void* paddr, ulong_t vaddr, int pagefileIndex) {
    ...
    for (i = 0; i < SECTORS_PER_PAGE; i++) {
        Block_Write(pageDev->dev,
                    pageDev->startSector + SECTORS_PER_PAGE * pagefileIndex + i,
                    paddr + i * SECTOR_SIZE);
...
void Page_Cleaner(void) {
    ...
    if (g_numTicks % PAGE_CLEARING_CYCLE != 0)
        return;
#endif
    //  Print("Page_Cleaner().\n");
