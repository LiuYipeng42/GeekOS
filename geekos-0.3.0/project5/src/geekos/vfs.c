...
static struct Paging_Device* s_pagingDevice = 0;
 ...
 * Adapter for Open().
 */
static int Do_Open_File(struct Mount_Point* mountPoint,
                        const char* path,
                        int mode,
                        struct File** pFile) {
...
int Close(struct File* file) {
    ...
    KASSERT(file != 0);
    KASSERT(file->ops != 0);
    KASSERT(file->ops->Close !=
            0); /* All filesystems must implement Close(). */
 ...
 */
struct File* Allocate_File(struct File_Ops* ops,
                           int filePos,
                           int endPos,
                           void* fsData,
                           int mode,
...
/*
 * Clone given file.
 * This makes a new File object which accesses the same
 * underlying data source as the original File.
 * This operation is used when spawning a new process -
 * the parent may specify open files for the child to
 * inherit as its standard input and output descriptors.
 * Params:
 *   file - the File object
 *   pClone - reference to variable where pointer where the
 *     clone File should be stored
 * Returns: 0 if successful, error code (< 0) if not
 */
int Clone_File(struct File* file, struct File** pClone) {
    if (file->ops->Clone == 0)
        return EUNSUPPORTED;
    else {
        int rc = file->ops->Clone(file, pClone);
        Debug("\nVFS Clone_File %p -> %p\n", file, *pClone);
        return rc;
    }
}
...
int Open_Directory(const char* path, struct File** pDir) {
    int rc = Do_Open(path, 0, pDir, &Do_Open_Directory);
    Debug("\nVFS Open_Directory %p\n", *pDir);
    /*if (rc != 0) { Print("Directory open failed with code %d\n", rc); }*/
    return rc;
}
...
void Register_Paging_Device(struct Paging_Device* pagingDevice) {
    ...
    Print("Registering paging device: %s on %s\n", pagingDevice->fileName,
          pagingDevice->dev->name);
...
/*
 * Set the Access Control List of a file or directory
 */
int SetAcl(char* path, int uid, int permissions) {
    char prefix[MAX_PREFIX_LEN + 1];
    const char* suffix;
    struct Mount_Point* mountPoint;
    ...
    if (!mountPoint->ops->SetAcl)
        return EUNSUPPORTED;

    return mountPoint->ops->SetAcl(mountPoint, suffix, uid, permissions);
}

/*
 * Set the Set User Identification of a file or directory
 */
int SetSetUid(char* path, int setUid) {
    char prefix[MAX_PREFIX_LEN + 1];
    const char* suffix;
    struct Mount_Point* mountPoint;
    ...
    if (!mountPoint->ops->SetSetUid)
        return EUNSUPPORTED;

    return mountPoint->ops->SetSetUid(mountPoint, suffix, setUid);
}
