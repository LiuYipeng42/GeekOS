 ...
 * SCHEDULER CHARACTERISTICS
       ...
       Multilevel Feedback Queue
               ...
               - Threads from queues with highest priority are always scheduled
 first (queue 0 has highest priority)
               - FirstComeFirstServed Policy in all queues
               - New threads are always put on end of queue with highest
 priority
               - if thread does not complete within quantum,
                               then the thread is put on the end of the next
 queue (lower priority)
               - thread will be promoted to the next queue with higher priority,
 if thread was blocked before
...
int Switch2SchedulingPolicy(int policy, int quantum) {
    ...
    switch (policy) {
        ...
        case SCHEDULE_MLF:  // switch to MLF
            ...
            // find the system idle process and put into queue with lowest
            // priority
...
void Make_Runnable(struct Kernel_Thread* kthread) {
    ...
    // for MLF schedulung: promote thread higher priority if thread was blocked
    // before
    if ((g_currentScheduleAlgorithm == SCHEDULE_MLF) &&
        (kthread->blocked == true) && (kthread->currentReadyQueue > 0)) {
...
int Move_All_Threads_To_Wait_Queue(int toQueue) {
    ...
    while (kthread) {
        ...
        if (currentWaitQueue != toQueue) {
            ...
            if (Is_Member_Of_Thread_Queue(&s_runQueue[currentWaitQueue],
                                          kthread)) {
