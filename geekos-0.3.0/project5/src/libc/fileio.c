...
DEF_SYSCALL(Stat,
            ...
            , SYSCALL_REGS_3)
DEF_SYSCALL(
    FStat, SYS_FSTAT, int, (int fd, struct VFS_File_Stat* stat), int arg0 = fd;
...
DEF_SYSCALL(Read_Entry,
            ...
            , SYSCALL_REGS_2)
DEF_SYSCALL(
    Read, SYS_READ, int, (int fd, void* buf, ulong_t len), int arg0 = fd;
    void* arg1 = buf;
    ulong_t arg2 = len;
    , SYSCALL_REGS_3)
DEF_SYSCALL(Write,
            SYS_WRITE,
            int,
            (int fd, const void* buf, ulong_t len),
            int arg0 = fd;
            const void* arg1 = buf;
            ulong_t arg2 = len;
            , SYSCALL_REGS_3)
...
DEF_SYSCALL(Seek, SYS_SEEK, int, (int fd, int pos), int arg0 = fd;
            ...
            , SYSCALL_REGS_2)
DEF_SYSCALL(
    Delete, SYS_DELETE, int, (const char* path), const char* arg0 = path;
    size_t arg1 = strlen(path);
    , SYSCALL_REGS_2)

DEF_SYSCALL(Create_Pipe,
            SYS_CREATEPIPE,
            int,
...
int Mount(const char* devname, const char* prefix, const char* fstype) {
    ...
    __asm__ __volatile__(SYSCALL : "=a"(rc) : "a"(num), "b"(&args));
    return rc;
}
