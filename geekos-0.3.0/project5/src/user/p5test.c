...
int ttestMount() {
    ...
    ret = Mount("ide1", "/d", "gosfs");
    Print("Mount returned %d ", ret);
    return ret;
}
...
int tReadEntry() {
    ...
    if ((retR < 0) || (strncmp(dirEntry.name, ".", 2) != 0) ||
    ...
    if ((retR < 0) || (strncmp(dirEntry.name, "d1", 2) != 0) ||
        (!dirEntry.stats.isDirectory))
        return -1;

    retR = Read_Entry(fd, &dirEntry);

    if ((retR < 0) || (strncmp(dirEntry.name, "d2", 2) != 0) ||
        (!dirEntry.stats.isDirectory))
        return -1;

    retR = Read_Entry(fd, &dirEntry);

    if ((retR < 0) || (strncmp(dirEntry.name, "f1", 2) != 0) ||
        (dirEntry.stats.isDirectory))
        return -1;
...
int tFiveMegs() {
    ...
    retS=Seek(fd, (4*1024)*8 + (4*1024)*1024+(4*1024)*1024*100+(1024*4)*200);
    if (retS < 0 ) return -1;

    retW = Write( fd, "Test", 5);
...
int main(int argc, char** argv) {
    ...
    // // 2
    doTest("Open-Inexistent File", tOpenInexistentFile, 1, &score, &totalTests,
           &successfulTests);
    // 3
    doTest("Creat", tCreat, 2, &score, &totalTests, &successfulTests);
    // 4
    doTest("Creat-Long Filename", tCreatLongFilename, 3, &score, &totalTests,
           &successfulTests);
    // 5
    ...
    // 7
    doTest("Read from WriteOnly", tReadFromWriteOnly, 3, &score, &totalTests,
           &successfulTests);
    // 8
    ...
    doTest("Delete Empty Directory", tDeleteEmptyDirectory, 3, &score,
           &totalTests, &successfulTests);
    // 22
    doTest("Delete Non-Empty Directory", tDeleteNonEmptyDirectory, 2, &score,
           &totalTests, &successfulTests);
