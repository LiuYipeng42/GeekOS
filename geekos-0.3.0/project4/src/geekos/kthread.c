...
/*static*/ void Setup_User_Thread(struct Kernel_Thread* kthread,
     ...
     *   the argument block
     */
    //  ("Create a new thread to execute in user mode");
    ulong_t eflags = EFLAGS_IF;
    unsigned csSelector = userContext->csSelector; /* CS 选择子 */
    unsigned dsSelector = userContext->dsSelector; /* DS 选择子 */
...
struct Kernel_Thread* Get_Next_Runnable(void) {
    ...
    //  ("Find a runnable thread from run queues");
...
struct Kernel_Thread* Lookup_Thread(int pid) {
    ...
    /*
     * : we could remove the requirement that the caller
