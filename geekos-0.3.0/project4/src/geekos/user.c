 ...
 * Params:
 * @param program 要读入内存缓冲区的可执行文件 - the full path of the program
 * executable file
 * @param command 用户执行程序执行时的命令行字符串 - the command, including name
 * of program and arguments
 * @param pThread 存放指向刚创建进程的指针 - reference to Kernel_Thread pointer
 * where a pointer to the newly created user mode thread (process) should be
 *     stored
 * @return
...
int Spawn(const char* program,
     ...
     */
    //  ("Spawn a process by reading an executable from a filesystem");
    int res;
    ...
    res = Load_User_Program(exeFileData, exeFileLength, &exeFormat, command,
                            &userContext);
    if (res != 0) {
        if (exeFileData != NULL)
            Free(exeFileData);
...
void Switch_To_User_Context(struct Kernel_Thread* kthread,
    ...
    // project4 Switch_To_User_Context
    /*
     * Hint: Before executing in user mode, you will need to call
     * the Set_Kernel_Stack_Pointer() and Switch_To_Address_Space()
     * functions.
     */
    //  ("Switch to a new user address space, if necessary");
