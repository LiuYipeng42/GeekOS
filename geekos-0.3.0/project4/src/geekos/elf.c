 ...
 * @param exeFileData 已装入内存的可执行文件所占用空间的起始地址 buffer
 * containing the executable file
 * @param exeFileLength 可执行文件长度 length of the executable file in bytes
 * @param exeFormat 保存分析得到的elf文件信息的结构体指针 structure describing
 * the executable's segments and entry address; to be filled in
 * @return 0 if successful, < 0 on error
 */
int Parse_ELF_Executable(char* exeFileData,
                         ulong_t exeFileLength,
                         struct Exe_Format* exeFormat) {
    //  ("Parse an ELF executable image");
