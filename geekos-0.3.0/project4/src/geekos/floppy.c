...
static struct Floppy_Parameters s_floppyParamsTable[] = {
    ...
    INVALID_FLOPPY_TYPE,     /* : 360K 5.25" */
    INVALID_FLOPPY_TYPE,     /* : 1.2M 5.25" */
    INVALID_FLOPPY_TYPE,     /* : 720K 3.5" */
    {80, 2, 18, 0x02, 0x1C}, /* 1.44M 3.5" */
    INVALID_FLOPPY_TYPE,     /* : 2.88M 3.5" */
};
#define NUM_FLOPPY_TYPES \
    (sizeof(s_floppyParamsTable) / sizeof(struct Floppy_Parameters))
 ...
 */
static void LBA_To_CHS(struct Floppy_Drive* drive,
                       int lba,
                       int* cylinder,
                       int* head,
                       int* sector) {
...
static void Wait_For_MRQ(uchar_t readyValue) {
    KASSERT(readyValue == FDC_STATUS_READY_READ ||
            readyValue == FDC_STATUS_READY_WRITE);
...
static void Start_Motor(int drive) {
    Out_Byte(FDC_DOR_REG, FDC_DOR_MOTOR(drive) | FDC_DOR_DMA_ENABLE |
                              FDC_DOR_RESET_DISABLE | FDC_DOR_DRIVE_SELECT(0));
}

static void Stop_Motor(int drive) {
    Out_Byte(FDC_DOR_REG, FDC_DOR_DMA_ENABLE | FDC_DOR_RESET_DISABLE |
                              FDC_DOR_DRIVE_SELECT(0));
}
...
static bool Reset_Controller(void) {
    ...
    /*
     * Enable fd0
     * : we might want to support drives other than 0 eventually
     */
    Start_Motor(0);
...
static int Floppy_Transfer(int direction,
                           int driveNum,
                           int blockNum,
                           char* buf) {
...
static void Floppy_Request_Thread(ulong_t arg) {
    ...
    for (;;) {
        ...
        if (request->type == BLOCK_READ)
            rc = Floppy_Read(request->dev->unit, request->blockNum,
                             request->buf);
        else
            rc = Floppy_Write(request->dev->unit, request->blockNum,
                              request->buf);
