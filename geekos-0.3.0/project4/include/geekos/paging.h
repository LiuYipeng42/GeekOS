...
#define PAGE_DIRECTORY_INDEX(x) (((unsigned int)(x) >> 22) & 0x3ff)  // 最高 10 位
#define PAGE_TABLE_INDEX(x) (((unsigned int)(x) >> 12) & 0x3ff) // 中间 10 位

#define PAGE_ALLIGNED_ADDR(x) (((unsigned int)(x)) >> 12)
#define PAGE_ADDR(x) (((unsigned int)(x)) << 12)
#define PAGE_ADDR_BY_IDX(x, y) \
    (((unsigned int)(x) << 22) + ((unsigned int)(y) << 12))
#define PAGE_IDNEX(x) (((unsigned int)(x) << 10) >> 22)
 ...
 */
typedef struct {
    uint_t present : 1;  //页在内存中是否存在
    uint_t flags : 4;    //页的访问权限
    uint_t accesed : 1;  //页是否被访问过
    uint_t reserved : 1;
    uint_t largePages : 1;          //页表大小
    uint_t globalPage : 1;          //是否为全局页表
    uint_t kernelInfo : 3;          //内核页表
    uint_t pageTableBaseAddr : 20;  //页表在内存中的地址
} pde_t;
...
typedef struct {
    ...
    uint_t flags : 4;          //页的访问权限
    uint_t accesed : 1;        //页是否被访问过
    uint_t dirty : 1;          //是否被修改过
    uint_t pteAttribute : 1;   //页大小
    uint_t globalPage : 1;     //是否位全局页
    uint_t kernelInfo : 3;     //内核页
    uint_t pageBaseAddr : 20;  //页在内存中的物理页号
} pte_t;
