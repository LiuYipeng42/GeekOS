...
/*static*/ void Setup_User_Thread(struct Kernel_Thread* kthread,
	...
	// 将 kthread 的 userContext 指针指向传入的 userContext
    Attach_User_Context(kthread, userContext);

    /* 初始化用户态进程堆栈，使之看上去像刚被中断运行一样 p149*/
    /* 分别调用 Push 函数将以下数据压入堆栈 */
    Push(kthread, dsSelector);                    /* DS 选择子 */ // 通过此选择子可以访问到 userContext->memory 中的数据
    Push(kthread, userContext->stackPointerAddr); /* 堆栈指针 */
    Push(kthread, eflags);                        /* Eflags */
    Push(kthread, csSelector);                    /* CS 选择子 */ // 通过此选择子可以访问到 userContext->memory 中的代码
...
/*
 * Start a user-mode thread (is.e., a process), using given user context.
...
`
    /* 建立用户态进程 */
    // 创建一个内核进程对象 Kernel_Thread
    struct Kernel_Thread* kthread = Create_Thread(PRIORITY_USER, detached);
    if (kthread == NULL) {
        return NULL;
    }

    // 将刚创建的 User_Context 对象 传入的 Kernel_Thread 对象的 userContext指针
    // 和初始化 Kernel_Thread 对象的堆栈页
